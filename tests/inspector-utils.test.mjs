...
} from '../src/lib/inspector-utils.ts';
...
describe('inspector utils', () => {
    ...
    test('applies regex filters only to the websocket payload', () => {
        ...
        expect(
            matchesTextFilter(
                ['ws://localhost:8787/inbox', 'localhost:3091/inbox-shared-worker.ts', 'ping'],
                excludeHeartbeat,
            ),
        ).toBe(false);
        expect(
            matchesTextFilter(
                ['ws://localhost:8787/inbox', 'localhost:3091/inbox-shared-worker.ts', 'pong'],
                excludeHeartbeat,
            ),
        ).toBe(false);
        expect(
            matchesTextFilter(
                ['ws://localhost:8787/inbox', 'localhost:3091/inbox-shared-worker.ts', '{"type":"message.created"}'],
                excludeHeartbeat,
            ),
        ).toBe(true);
    });
