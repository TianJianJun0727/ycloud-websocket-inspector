...
5. 打开使用 WebSocket 的业务页面，再点击工具栏中的扩展图标。
...
- 插件自动扫描普通页面、Dedicated Web Worker 与 SharedWorker 调试目标，并统一捕获其中的 WebSocket。
- 连接列表通过“页面”“Web Worker”“SharedWorker”徽标区分连接来源。
...
- 文本筛选支持普通包含匹配，也支持 `/pattern/flags` 形式的正则表达式。普通文本会匹配当前连接的 URL、运行环境 URL 和消息正文；正则只匹配消息正文，flags 按输入原样生效。
...
- 如果插件在 WebSocket 已经建立之后才连接，仍可能捕获后续帧，但 URL 可能要等业务
  ...
  无法唯一对应时使用运行环境地址和 request ID 作为稳定标识，不猜测 URL。
- 调试期间不要同时为同一个页面或 Worker 打开 DevTools，它们会竞争同一调试目标。
...
- 后台以最多 64 条或 70ms 为一个批次向 React UI 传输，降低高流量时的扩展消息与
  重渲染开销。
- 顶部“存储异常”仅表示扩展 IndexedDB 初始化或写入失败；页面或 Worker 调试连接、扫描
...
视觉验证；实际捕获能力必须在加载扩展后，以真实页面或 Worker WebSocket 进行验证。

## UI 开发

项目使用 WXT、React、TypeScript 和 TailwindCSS v4。首次开发先运行 `bun install`，启动开发模式：
...
本工具通过 Chrome 调试目标提供显式消息模拟，不采用页面脚本注入、消息阻断、自动去重或
高流量自动停监控能力，避免静默改变或丢弃用于一致性验证的原始 WebSocket 消息。
