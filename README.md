...
- 数据按 WebSocket 连接实例分别保存在扩展后台内存中，不写 localStorage、IndexedDB，也不上传。
- 最多保留 20,000 条或约 64 MB payload；超过后从最早消息开始淘汰。
- 单个 WebSocket 最多保留 5,000 条，避免一个高流量连接挤掉其他连接的全部记录。
- 单条 payload 最多保留 1 MB，超出部分标记为截断。
- 页面表格只渲染筛选结果中最新 800 条，导出包含内存内全部筛选结果。
